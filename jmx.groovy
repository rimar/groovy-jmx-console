...
def invokeMbean() {
    ...
    def thePrms = new ArrayList<String>(params.keySet()).minus([invoke_op, invoke_mbean])

    MBeanOperationInfo info = beanInfo.getOperations().find {op ->
        def prms = op.signature.collect { sign ->
            sign.name  + "_" + sign.type
        }

        op.getName().equals(opName) && prms.intersect(thePrms).size() == op.signature.size()
    }
...
private def operationForm(mbean, MBeanOperationInfo op, name) {
    ...
    html.form(action: home_url, method: "post") {
        ...
        op.signature.each { sign ->
            input(type: "hidden", name: sign.name + "_" + sign.type, value: sign.type)
        }
